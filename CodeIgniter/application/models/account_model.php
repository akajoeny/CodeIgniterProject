...
class Account_model extends CI_Model {
	...
	{
		$account = array();
		$account['email'] = $this->db->get_where('a3m_account', array('email' => $email))->row_array();
	...
	/**
	 * Get account_id by email
	 *
	 * @access public
	 * @param string $email
	 * @return int account_id int
	 */
	function get_account_id($email)
	{
		$account_array = Array();
		$account_array = $this->db->get_where('a3m_account', array('email' => $email))->row_array();
		return $account_array['id'];	
	}
	 ...
	 * @return void
	 */
	function update_last_signed_in_datetime($account_email)
	{
		$this->load->helper('date');
		
		$this->db->update('a3m_account', array('lastsignedinon' => mdate('%Y-%m-%d %H:%i:%s', now())), array('email' => $account_email));
	}
